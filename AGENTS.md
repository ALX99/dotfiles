...
.config/ holds per-app configs (Neovim, river, tmux, etc.) that get symlinked into `~/.config`. Shell entrypoints live in `home/` (`home/.bashrc`, `home/.aliasrc`, `home/.profile`), and `home/.gitconfig` is linked to `~/.gitconfig`. Executable helpers belong in `.local/bin/` (one file per command, lowercase names). System-level tweaks live under `misc/` (systemd configs in `misc/systemd-config/`, pacman hooks in `misc/pacman-hooks/`, keymaps in `misc/keymaps/`). Keep screenshots or diagrams in the root-level PNG artifacts. Submodules (`.local/bin/adb-uninstall`, `.config/mpv/scripts/subs2srs`, `misc/ghostty-shaders/`) must be kept in sync with `git submodule update --init --recursive`.
