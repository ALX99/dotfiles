...
test("a plan retains its preflight hasher and revalidation strategy", async () => {
	await withWorkspace(async (root) => {
		const hashSource = (content: string) => `fixture:${content.length}`;
...
+planned`),
		...
		try {
			await assert.rejects(applyPreflightedPatch(plan), (error) => {
...
async function injectFailure<T>(
	...
	run: (operations: PatchEngineOperations) => Promise<T>,
...
test("preflight failure injection leaves the workspace and unrelated files unchanged", async () => {
	await withWorkspace(async (root) => {
		...
		for (const point of ["source_read", "hash"] as const) {
			await injectFailure(point, async (operations) => {
				await assert.rejects(preflightPatch(patch, { workspaceRoot: root, operations }), /injected/);
			});
		...
		const controller = new AbortController();
		const operations: PatchEngineOperations = {
			atFailurePoint: (point) => {
				if (point === "hash") controller.abort();
			},
		};
		await assert.rejects(
			preflightPatch(patch, { workspaceRoot: root, signal: controller.signal, operations }),
			(error) => error instanceof PatchEngineError && error.code === "cancelled",
		);
		assert.equal(await readFile(source, "utf8"), "before\n");
		assert.equal(await readFile(unrelated, "utf8"), "keep\n");
...
test("staging and publication failure injection leaves no applied change or unrelated modification", async () => {
	...
	for (const fixture of cases) {
		await withWorkspace(async (root) => {
			...
			const unrelated = path.join(root, "unrelated.txt");
			await writeFile(unrelated, "keep\n");
			const error = await injectFailure(fixture.point, async (operations) => {
				const plan = await preflightPatch(wrap(fixture.patch), { workspaceRoot: root, operations });
				return expectWriteFailure(() => applyPreflightedPatch(plan));
			});
...
test("rollback, cleanup, and cancellation injection report recovery state without touching unrelated files", async () => {
		...
		await withWorkspace(async (root) => {
			...
			const patch =
				point === "rollback_restore"
					...
					: "*** Add File: first.txt\n+first\n*** Update File: source.txt\n@@\n-before\n+after";
			const error = await injectFailure(point, async (operations) => {
				const plan = await preflightPatch(wrap(patch), { workspaceRoot: root, revalidateSource, operations });
				return expectWriteFailure(() => applyPreflightedPatch(plan));
			});
	...
	await withWorkspace(async (root) => {
		...
		const error = await injectFailure("rollback_restore", async (operations) => {
	...
	await withWorkspace(async (root) => {
		...
		await writeFile(unrelated, "keep\n");
		await writeFile(path.join(root, "source.txt"), "before\n");
		await injectFailure("journal_cleanup", async (operations) => {
...
test("bounded reads reconstruct a file from short chunks", async () => {
	const source = Buffer.from("complete source despite short reads");
	const positions: number[] = [];
	const result = await readBoundedFile(async (buffer, offset, length, position) => {
		positions.push(position);
		const chunk = source.subarray(position, position + Math.min(3, length));
		chunk.copy(buffer, offset);
		return chunk.length;
	});
	assert.equal(result.toString("utf8"), source.toString("utf8"));
