...
function extractNativePatchMetadata(source, nativePatchPath) {
	const version = /const EXPECTED_VERSION = "([^"]+)";/.exec(source)?.[1];
	if (!version) fail(`cannot find EXPECTED_VERSION in ${nativePatchPath}`);

	const targets = [
		...source.matchAll(
			/targetRelative:\s*"([^"]+)",\s*beforeSha256:\s*"([a-f0-9]{64})",\s*afterSha256:\s*"([a-f0-9]{64})"/g,
		),
	].map((match) => ({
		targetRelative: match[1],
		beforeSha256: match[2],
		afterSha256: match[3],
	}));
	if (targets.length === 0) fail(`cannot find patch target hashes in ${nativePatchPath}`);
...
export async function checkPiCompatibility(options = {}) {
	...
	const nativePatchPath = resolve(
		options.nativePatchPath ?? join(repoRoot, "misc/pi-patches", `apply-pi-ai-${expectedVersion}.mjs`),
	);
	const nativePatchSource = await readFile(nativePatchPath, "utf8").catch((cause) => {
		fail(`cannot read native patch at ${nativePatchPath}: ${cause instanceof Error ? cause.message : String(cause)}`);
	});
	const nativePatch = extractNativePatchMetadata(nativePatchSource, nativePatchPath);
