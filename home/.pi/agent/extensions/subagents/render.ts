...
import { parseChildExecutionContext, type ReadonlyNestedRunDetails, type ReadonlyRunDetails } from "./process.ts";
...
export function taskPreview(s: string): string {
	return clipTerminalText(s, 80, "...");
}

function clipLine(s: string, max: number): string {
	return clipTerminalText(s, max);
}
...
export function renderCallHeader(
	...
	if (args.message) {
		if (!expanded) {
			...
			c.addChild(new Text(theme.fg("text", sanitizeTerminalBlock(args.message)), 0, 0));
		}
	}
}
...
export function renderManagementCall(
	...
	c.addChild(
		new Text(
			`${theme.fg("toolTitle", theme.bold(sanitizeTerminalText(toolName)))}${theme.fg("muted", target)}${mode}`,
			0,
			0,
		),
	);
	if (message) {
		const body = expanded ? sanitizeTerminalBlock(message) : taskPreview(message);
		c.addChild(new Text(theme.fg(expanded ? "text" : "dim", `  ${body}`), 0, 0));
	}
	return c;
}

export function renderAgentSummaries(
	toolName: string,
	summaries: readonly AgentSummary[],
	expanded: boolean,
	theme: Theme,
...
export function renderResultBlock(details: ReadonlyRunDetails, options: RenderOptions, theme: Theme): Container {
	...
	const icon = isRunning ? theme.fg("warning", "⟳") : failed ? theme.fg("error", "✗") : theme.fg("success", "✓");

	const headerParts = [
		`${icon} ${theme.fg("toolTitle", theme.bold(sanitizeTerminalText(details.taskName || details.agent)))}`,
	...
	if (options.expanded) {
		...
		headerParts.push(
			theme.fg(
				"muted",
				`· ${sanitizeTerminalText(details.profile)} · ${sanitizeTerminalText(details.model)} · thinking=${sanitizeTerminalText(details.effectiveThinking)}`,
			),
		);
		headerParts.push(theme.fg("dim", `· ${details.toolCount} tools · ${elapsed}${isRunning ? " running" : ""}`));
	} else {
		headerParts.push(theme.fg("muted", `· ${sanitizeTerminalText(details.profile)}`));
	...
	if (!isRunning && details.finalText) {
		...
		c.addChild(new Text(theme.fg("toolOutput", sanitizeTerminalBlock(preview)), 0, 0));
	}
