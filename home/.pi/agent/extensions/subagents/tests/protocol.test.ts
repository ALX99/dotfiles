...
test("protocol accepts harmless response metadata and unknown UI methods", () => {
	const response = parseRpcRecord({
		type: "response",
		id: "x",
		success: true,
		data: { ok: true },
	...
	if (response.kind === "response" && response.response.success) {
		assert.deepEqual(response.response.data, { ok: true });
	}

	const uiRequest = parseRpcRecord({
		type: "extension_ui_request",
		id: "ui-future",
		method: "futureWidgetMethod",
		futureMetadata: true,
	});
...
test("unknown UI methods and response extras do not corrupt the transport", async (t) => {
	const script = String.raw`
let buffer = '';
