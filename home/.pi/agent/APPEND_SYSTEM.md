...
- Write comments only to explain the current code and its enduring constraints. Do not mention removed, absent, or superseded code, configuration, dependencies, or approaches, including why they were removed. Do not add negative assertions that merely enumerate unrelated behavior or components the code does not affect. When historical context or a boundary is essential, document the current constraint rather than the removed or unaffected alternative.
